...
@interface SportsDetailsController () <AMapLocationManagerDelegate>
...
//            NSLog(@"时间：%d", time);
        ...
        [_motionManager startDeviceMotionUpdatesToQueue:[NSOperationQueue  currentQueue] withHandler:^(CMDeviceMotion * _Nullable motion, NSError * _Nullable error) {
            ...
            [_db executeUpdate:@"insert into acceleration(x,y,z,time) values(?,?,?,?);", @(motion.userAcceleration.x), @(motion.userAcceleration.y), @(motion.userAcceleration.z), @(timeStamp)];
        }];
        //设备当前的活动状态
         if ([CMMotionActivityManager isActivityAvailable]) {
            [_motionActivityManager startActivityUpdatesToQueue:[NSOperationQueue mainQueue] withHandler:^(CMMotionActivity * _Nullable activity) {
                ...
                if (![userActivity isEqualToString:@""]) {
                    [_db executeUpdate:@"insert into activity(activity, confidence, time) values(?,?,?);", userActivity, activity.confidence, @(timeStamp)];
                }
            }];
        } else {
            DDLogWarn(@"error");
